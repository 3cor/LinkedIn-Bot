...
def validate_questions() -> None | ValueError | TypeError:
    ...
    check_string(current_company, "current_company")
    check_string(current_title, "current_title")
    check_string(confidence_level, "confidence_level")
    check_string(school_name, "school_name")
    check_string(degree, "degree")
    check_string(specialization, "specialization")
    check_string(discipline, "discipline")
    check_string(referral_source, "referral_source")

    # Additional application questions
    check_string(gender_identity, "gender_identity")
    check_string(require_visa_sponsorship, "require_visa_sponsorship", ["Yes", "No"])
    check_string(work_authorization_us, "work_authorization_us", ["Yes", "No", "Unknown"])
    check_string(race_ethnicity, "race_ethnicity")
    check_string(current_country, "current_country")
    check_string(open_to_relocation, "open_to_relocation", ["Yes", "No"])
    check_string(preferred_hub_city, "preferred_hub_city")
    check_string(hiring_manager_message, "hiring_manager_message")
    check_string(reasonable_accommodation, "reasonable_accommodation")
    check_string(protected_veteran_status, "protected_veteran_status")
    check_string(relatives_in_company, "relatives_in_company", ["Yes", "No"])
    check_string(relatives_names, "relatives_names")
    check_string(acknowledge_privacy_policy, "acknowledge_privacy_policy")
